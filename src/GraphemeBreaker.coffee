...
# Gets the grapheme breaking class for the given code point
getCharClass = (code) ->
  low = 0
  high = classes.length
...
exports.nextBreak = (string, index = 0) ->
  ...
  prev = getCharClass codePointAt(string, index)
  for i in [index + 1...string.length] by 1
    ...
    next = getCharClass codePointAt(string, i)
...
exports.previousBreak = (string, index = string.length) ->
  ...
  index--
  next = getCharClass codePointAt(string, index)
  for i in [index - 1..0] by -1
    ...
    prev = getCharClass codePointAt(string, i)
    if shouldBreak prev, next
      return i + 1
